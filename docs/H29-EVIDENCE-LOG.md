...
| The previous-invoice-hash value a chain's FIRST document must carry | B5 records the TRANSFORM ZATCA uses (SHA-256); it does not record the value that position takes when there is no previous document. Guessing would put a fabricated value into a field an authority checks. The element is left empty and the adapter reports `pih-initial-unknown` as a warning on every first document, so the gap travels with the document rather than being discovered later. Owner action OA-7. |
