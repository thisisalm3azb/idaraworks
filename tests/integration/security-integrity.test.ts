...
import { installFinanceSetup, createBankAccount, recordMoneyTransaction } from "@/modules/finance/service";
...
beforeAll(async () => {
  ...
  orgA = await createOrgForUser(userA, { name: `SecInt ${run}`, country: "AE", baseCurrency: "AED" });
  ...
  bankId = (await createBankAccount(A(), "owner", { name: "Main bank", kind: "bank", glCode: "1111" })).id;
  cashId = (await createBankAccount(A(), "owner", { name: "Till", kind: "cash", glCode: "1101" })).id;
}, 600_000);
...
describe("payments (F-27)", () => {
  it("two concurrent requests with the same key record ONE payment and both answer with it", async () => {
    ...
    const input = { method: "cash", paymentDate: "2026-06-01", amountMinor: 12_345, currency: "AED", idempotencyKey: key };
    ...
    const rows = await owner`select count(*)::int as n from public.payment where org_id = ${orgA} and idempotency_key = ${key}`;
    expect(rows[0]!.n).toBe(1);
  }, 60_000);

  it("two requests with different keys are two deliberate payments", async () => {
    const one = await recordPayment(A(), "owner", { method: "cash", paymentDate: "2026-06-02", amountMinor: 100, currency: "AED", idempotencyKey: `pmt-${randomUUID()}` });
    const two = await recordPayment(A(), "owner", { method: "cash", paymentDate: "2026-06-02", amountMinor: 100, currency: "AED", idempotencyKey: `pmt-${randomUUID()}` });
...
describe("expenses (F-27, 0143)", () => {
  it("a retried expense with the same key is replayed, not recorded twice", async () => {
    ...
    const input = { categoryKey, description: "Office supplies", expenseDate: "2026-06-03", amountMinor: 5_000, vatAmountMinor: 250, idempotencyKey: key };
    const first = await createExpense(A(), "owner", input);
    const second = await createExpense(A(), "owner", input);
    expect(second.id).toBe(first.id);
    expect(second.reference).toBe(first.reference);
    const rows = await owner`select count(*)::int as n from public.expense where org_id = ${orgA} and idempotency_key = ${key}`;
    expect(rows[0]!.n).toBe(1);
    // and exactly one cost posting for it
    const entries = await owner`select count(*)::int as n from public.journal_entry where org_id = ${orgA} and source_type = 'expense' and source_id = ${first.id} and event_key = 'recorded'`;
    expect(entries[0]!.n).toBe(1);
  }, 120_000);

  it("different keys record two expenses", async () => {
    const one = await createExpense(A(), "owner", { categoryKey, description: "A", expenseDate: "2026-06-03", amountMinor: 10, vatAmountMinor: 0, idempotencyKey: `exp-${randomUUID()}` });
    const two = await createExpense(A(), "owner", { categoryKey, description: "B", expenseDate: "2026-06-03", amountMinor: 10, vatAmountMinor: 0, idempotencyKey: `exp-${randomUUID()}` });
    expect(one.id).not.toBe(two.id);
  }, 120_000);
});
...
describe("bank vouchers (F-27)", () => {
  it("a retried voucher with the same key is replayed, not posted twice", async () => {
    ...
    const input = { kind: "transfer", bankAccountId: bankId, counterBankAccountId: cashId, txnDate: "2026-06-04", amountMinor: 7_000, memo: "float", idempotencyKey: key };
    const first = await recordMoneyTransaction(A(), "owner", input);
    const second = await recordMoneyTransaction(A(), "owner", input);
    expect(second.id).toBe(first.id);
    const rows = await owner`select count(*)::int as n from public.money_transaction where org_id = ${orgA} and idempotency_key = ${key}`;
    expect(rows[0]!.n).toBe(1);
  }, 120_000);
});
