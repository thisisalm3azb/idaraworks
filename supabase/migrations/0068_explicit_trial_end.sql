-- 0068_explicit_trial_end (adversarial-review CRITICAL fix): trial deadlines must be EXPLICIT.
--
-- The sweep previously derived a trial deadline for trial_end-NULL orgs from
-- period_start + 14d (windows.ts effectiveTrialEnd fallback). That armed a time bomb: the two
-- protected production orgs (Alpha Marine, TESTING — billing_state 'trialing', trial_end NULL,
-- created 2026-07-11..15) would have been silently downgraded growth→free by ANY sweep run after
-- ~2026-07-25 (integration suites against the hosted DB, or the Inngest cron once provisioned).
--
-- New contract, both halves in this migration + the paired windows.ts change:
--   * org creation STAMPS an explicit trial_end (now() + 14 days, v1 §13 default) — real orgs'
--     trials expire exactly as designed;
--   * trial_end NULL now genuinely means NO DEADLINE — the code fallback is deleted, so the
--     pre-0068 rows (exactly the two protected orgs) are permanently exempt from trial expiry
--     WITHOUT modifying them. Deliberately NO backfill of existing rows.
--
-- 5th revision of create_org_with_owner via CREATE OR REPLACE (0003→0004→0005→0007→here);
-- body identical to 0007 except the org_plan_state insert gains trial_end. Forward-only.
...
begin
  ...
  -- Default plan: full-featured Growth trial (v1 §13) with an EXPLICIT deadline (0068).
  insert into public.org_plan_state (org_id, plan_key, billing_state, trial_end)
  values (v_org_id, 'growth', 'trialing', now() + interval '14 days');
