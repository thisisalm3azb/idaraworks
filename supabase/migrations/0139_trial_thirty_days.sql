-- 0139 — a 30-day free trial for every NEW workspace (owner decision, 2026-09-20).
--
-- WHY: the product promises "30 days free, no credit card required". The
-- trial deadline is stamped by app.create_org_with_owner at creation (0068), so
-- the promise must be made there, not only in copy.
--
-- WHAT: same function body as 0068 with one change — trial_end is now() + 30 days.
-- Existing workspaces are NOT touched: their stamped trial_end stands until the
-- owner decides otherwise (a separate, explicit decision).
--
-- Rollback note: re-create the function from 0068_explicit_trial_end.sql.
...
begin
  ...
  -- Default plan: full-featured Growth trial with an EXPLICIT deadline — 30 days (0139).
  insert into public.org_plan_state (org_id, plan_key, billing_state, trial_end)
  values (v_org_id, 'growth', 'trialing', now() + interval '30 days');

  -- Atomic audit (owner context): org creation is recorded in the same
