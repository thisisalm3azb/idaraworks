...
- **A-B7 — OWNER DECISION (2026-07-11): Supabase region = Seoul (`ap-northeast-2`, `aws-1` pooler) — KEPT.** Supersedes AR-9's Mumbai recommendation; closes OA-2's region confirmation and doc 10 #43's "hosting region recorded". Accepted MVP trade-off: higher Gulf latency in exchange for not disturbing the existing canonical dev environment. Revisited **only** if real pilot telemetry demonstrates database latency as a production problem — i.e., via the freeze's change-control justification #2 (proven scalability/reliability issue), with the doc 11 §11 performance budgets as the measuring stick. Do not recreate or migrate the project.
- **A-B8 — OWNER DECISION (Phase E close): VC-4 literal in-Lambda run accepted as a documented residual.** The deployed-preview EXIF/GPS strip probe could not run because Vercel SSO deployment protection correctly blocked unauthenticated automation, and the owner declined to weaken protection solely for the test. VC-4's substance is met by equivalent evidence: EXIF/GPS removal passes in Linux CI on `@img/sharp-linux-x64` — the identical Sharp binary the Vercel Node runtime loads on Lambda; the full hosted Supabase Storage flow passed 69/69; the production build containing the image-processing worker (processImage → sharp) passed. **Revisit only when** an authenticated automation-bypass mechanism is already required for testing, OR deployed-runtime evidence shows a Sharp-specific failure. No architecture/freeze change.
