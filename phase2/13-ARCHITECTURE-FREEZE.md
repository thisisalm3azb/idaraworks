...
| 2026-07-11 | **Hosting region DECIDED (closes OA-2 / AR-9 / doc 10 #43): Supabase stays in Seoul (`ap-northeast-2`)** — accepted MVP trade-off (higher Gulf latency vs. preserving the canonical dev environment); no recreation or migration. Revisit only via change-control justification #2 (proven scalability/reliability issue from real pilot telemetry against the performance budgets). | Open-decision closure | Owner decision | Owner |
