...
**S9 BUILD LOG (local commits on top of e6b4306; NOTHING PUSHED yet — push+deploy gated on S9 gate-completion):**
- a8bb479 — migrations **0052** (org_plan_state: reconcile billing_state CHECK +purge_pending/+purged; add provider/customer/subscription id, interval, currency, trial_end/grace_until/suspend_at/purge_at, cancel_at_period_end, scheduled_plan_key, legal_hold; **plan_price** price-book minor-units placeholder-seeded AED+USD ×3 tiers ×month/year) + **0053** (subscription_event webhook inbox, platform-only; DEFINER sole-writer `app.record_subscription_event`/`app.advance_subscription`/`app.mark_subscription_event_processed`, assert_platform_task-guarded, 'purged' terminal). **Applied to hosted → 0000-0053.**
- 42e0857 — pure state machine `src/modules/subscription/machine.ts` (BILLING_STATES, canTransition, nextForEvent = v1 §13 dunning ladder) + unit 12/12.
- 70cbe8d — `src/platform/billing/adapter.ts` (BillingProvider interface, fakeBillingProvider w/ HMAC signEvent+verify, disabledBillingProvider prod-default, getBillingProvider) + unit 5/5. Env: BILLING_PROVIDER (fake|disabled), BILLING_FAKE_WEBHOOK_SECRET.
- Gates so far: typecheck clean, S9 unit 17/17. HEAD 70cbe8d. Hosted 0000-0053, next migration **0054**.
- 5f7c0aa — **SUBSCRIPTION LIFECYCLE MILESTONE (proven vs hosted 7/7).** migrations **0054** (usage_event append-only meter + `app.record_platform_audit` tenant-visible-audit-from-platform) + **0055** (`app.resolve_subscription_org` DEFINER). `src/modules/subscription/service.ts` (processSubscriptionWebhook = verify HMAC→idempotent inbox→resolve org→machine→advance_subscription→tenant audit→mark processed; startCheckout/cancelSubscription/readSubscription owner actions; assertTenantWritable FR-9). `src/app/api/billing/webhook/route.ts`. authz billing.view(owner/admin/accounts)+billing.manage(owner) both matrices. SubscriptionSignal moved to platform/billing/adapter (boundary fix). Env BILLING_PROVIDER. **Hosted 0000-0055, next 0056.** HEAD 5f7c0aa. S9 unit now part of 299 unit + s9 integ 7/7.
  - NOTE: s9-subscription.test.ts leaves an "S9 Org" in hosted (afterAll doesn't self-delete) — final s7-cleanup sweeps it.
...
- Usage metering wiring (usage_event helpers recordUsage/getUsage) + a metered-limit consumer.
- Telemetry: migration (per-tenant egress/storage/function-cost + per-card engagement) + surfaces. (Or document as pilot-telemetry-lite.)
- Notifications: dunning/trial-ending/limit-approaching/limit-reached/plan-changed/cancellation/grace/recovery/purge-warning (redacted bodies).
- Commercial admin (owner): plan/price-book view+edit (platform write-path DEFINER for owner edits of plan_price/overrides), subscription inspection, D1-activation checklist. Platform-vs-org separation.
- UI en/ar/RTL/375px: owner billing/plan admin, subscription+meter settings, upgrade/downgrade/cancel, **impersonation persistent banner**, **disabled-checkout "activation unavailable" state**, telemetry. i18n keys en+ar.
- Tests: bleed seeders for the 8 new tables (plan_price, subscription_event, usage_event, org_plan_state new cols, platform_staff, impersonation_session, dunning_attempt, reconciliation), authz-matrix parity (done), webhook out-of-order/dup (done), reconciliation, downgrade-never-blocks-reads, usage races, notification-redaction, e2e, prod smoke.
- Events/workers registry parity. DPA/PDPL posture doc (doc 10 #43). Add doc-10 items #51 webhook-sig-verify + #52 no-card-data/PCI (both implemented; document).
- THEN: adversarial multi-lens review (20 dims) + fixes → full gates (format/lint/typecheck/unit/hosted-integration/build/e2e/CI) → push → CI green → deploy → prod smoke → **Arabic s9-prod-demo** (trial→paid→past_due→recovery + support session in tenant audit, fake provider, 375px RTL) → cleanup (s7-cleanup sweeps S9 synthetic orgs) → completion report (distinguish prod-operational / D1-gated / credential-gated / owner-actions) → STOP (no S10/S11).
**RESUME — ORIGINAL NEXT STEPS (superseded by BUILT/STILL-TO-BUILD above):**
1. `src/modules/subscription/service.ts` — webhook processor (verify sig → record_subscription_event idempotent → nextForEvent → advance_subscription → audit via command()/platform path → mark_processed) + owner actions (startCheckout, cancelSubscription, changePlan → call adapter; set intent, state change arrives via webhook) + readSubscription (plan/state/meter for UI).
2. Webhook API route `src/app/api/billing/webhook/route.ts` (platform, no-tenant client; verify sig; 200 even on duplicate; never trust unverified).
3. Entitlement ENFORCEMENT on billing_state (suspended/cancelled/past_due→grace = block ADDs never reads, FR-9) — extend resolve.ts consumers / add a guard. Cross-instance cache invalidation on billing events.
4. Migrations 0054 usage_meter, 0055 dunning_attempt, 0056 impersonation_session (consent+banner+dual-log+break-glass), 0057 reconciliation+telemetry, 0058 platform write-path DEFINER for owner plan/override edits.
5. Services: entitlement enforcement, upgrade(immediate+proration)/downgrade(period-end), impersonation (consent+banner+dual-log to tenant audit), usage metering.
6. Workers: dunning, purge(legal_hold-checked), reconciliation, cache-invalidate. Events registry + inngest + defineOrgFunction.
7. authz actions BOTH matrices (billing.manage owner-only, impersonation.start platform-staff, etc.); platform-staff concept (NONE exists — introduce minimally). Notifications (dunning/trial/limit/purge redacted).
8. UI en/ar/RTL/375px: owner billing/plan admin, subscription+meter settings, upgrade/downgrade/cancel, impersonation banner, disabled-checkout state, telemetry dashboards.
9. Tests (unit+hosted integration+bleed+authz+webhook idempotency/out-of-order/signature+reconciliation+downgrade-never-blocks-reads+impersonation dual-log), s9-prod-demo (Arabic: trial→paid→past_due→recovery + support session in tenant audit), review, gates, deploy, cleanup, report.
**Key decisions locked:** provider seam disabled in prod (D1); org_plan_state writes ONLY via DEFINER platform path; suspended=read-only never lockout (FR-9); prices placeholder; platform-staff model is greenfield.
