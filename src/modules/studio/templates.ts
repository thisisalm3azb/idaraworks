...
export const BUILT_IN_TEMPLATES: readonly PlanTemplate[] = [
  {
    ...
    nodes: [
      ...
      },
      {
        ...
        title: "Lamination",
        durationDays: 10,
        estimateOptimisticDays: 8,
        estimatePessimisticDays: 15,
        x: 300,
        y: 200,
      },
      {
        key: "s3",
        nodeType: "task",
        title: "Below deck rigging",
        durationDays: 6,
        estimateOptimisticDays: 5,
      ...
      },
      {
        key: "s4",
        nodeType: "task",
        title: "3-part assembly",
        durationDays: 4,
        estimateOptimisticDays: 3,
        estimatePessimisticDays: 7,
        x: 780,
        y: 200,
      },
      {
      ...
      },
      {
        ...
        title: "Sea trial",
        durationDays: 2,
        estimateOptimisticDays: 1,
        estimatePessimisticDays: 4,
        x: 1020,
        y: 380,
      },
      { key: "m", nodeType: "milestone", title: "Delivery", x: 1260, y: 380 },
      {
        ...
        data: {
          ...
          impact: 4,
          response: "mitigate",
          mitigation: "Second supplier quoted",
        },
        x: 300,
        y: 560,
      },
      {
        key: "r2",
        nodeType: "risk",
        title: "Weather window for sea trial",
        data: { likelihood: 2, impact: 3, response: "accept" },
        x: 1020,
        y: 560,
      },
      {
        key: "d1",
        nodeType: "decision",
        title: "Engine option confirmed?",
        data: {
          question: "Which engine option?",
          options: [{ label: "Standard" }, { label: "Upgraded" }],
        },
        x: 540,
        y: 560,
      },
  ...
  },
  {
    ...
    nodes: [
      ...
      { key: "m", nodeType: "milestone", title: "Handover", x: 860, y: 200 },
      {
        key: "r",
        nodeType: "risk",
        title: "Hidden damage found",
        data: {
          likelihood: 3,
          impact: 3,
          response: "mitigate",
          mitigation: "Contingency days agreed",
        },
        x: 320,
        y: 480,
      },
  ...
  },
  {
    ...
    nodes: [
      { key: "o", nodeType: "objective", title: "Season-ready fleet", x: 500, y: 20 },
      {
        key: "k1",
        nodeType: "key_result",
        title: "Every hull sea-trialled by the target date",
        data: { target: 100, unit: "percent" },
        x: 100,
        y: 200,
      },
      {
        key: "k2",
        nodeType: "key_result",
        title: "Rework below 5% of hours",
        data: { target: 5, unit: "percent" },
        x: 500,
        y: 200,
      },
      {
        key: "k3",
        nodeType: "key_result",
        title: "No open severe risks at launch",
        data: { target: 0, unit: "count" },
        x: 900,
        y: 200,
      },
      {
      ...
      },
      {
        key: "i2",
        nodeType: "initiative",
        title: "Quality gates at each stage",
        durationDays: 15,
        x: 500,
        y: 380,
      },
      {
        key: "i3",
        nodeType: "initiative",
        title: "Risk reviews every fortnight",
        durationDays: 30,
        x: 900,
        y: 380,
      },
...
async function orgTemplates(ctx: Ctx): Promise<PlanTemplate[]> {
  ...
  const parsed = z
    .array(PlanTemplate)
    .safeParse((rows[0]?.value as { templates?: unknown })?.templates ?? []);
...
export const CreateFromTemplateInput = z.object({
  ...
  /** Where the plan starts; the engine needs one anchor. Defaults to the next Monday. */
  startDate: z
    .string()
    .regex(/^\d{4}-\d{2}-\d{2}$/)
    .optional(),
...
export async function createPlanFromTemplate(
  ...
  // Estimates ride the update path (the draft columns from 0109), and the
  // activities nothing depends on get the anchor date so the engine can place
  // the whole network from it.
  const { updateNode } = await import("./graph");
  const hasPredecessor = new Set(
    tpl.edges.filter((e) => e.edgeType === "dependency").map((e) => e.to),
  );
  ...
  const anchor = input.startDate ?? nextMonday();
  for (const n of tpl.nodes) {
    const patch: Record<string, unknown> = {};
    if (n.estimateOptimisticDays !== undefined) {
      patch.estimateOptimisticDays = n.estimateOptimisticDays;
    }
    if (n.estimatePessimisticDays !== undefined) {
      patch.estimatePessimisticDays = n.estimatePessimisticDays;
    }
    if (activity.has(n.nodeType) && !hasPredecessor.has(n.key) && n.durationDays !== undefined) {
      patch.startDate = anchor;
    }
    if (Object.keys(patch).length === 0) continue;
    await updateNode(ctx, archetype, { nodeId: ids.get(n.key)!, ...patch });
  }
  let edges = 0;
...
export async function saveAsTemplate(
  ...
  if (input.key.startsWith("builtin."))
    throw new StudioError("that key is reserved", "invalid_state");
    ...
    async (tx) => {
      ...
      if (next.length > MAX_ORG_TEMPLATES) {
        throw new StudioError(
          `at most ${MAX_ORG_TEMPLATES} organisation templates`,
          "invalid_state",
        );
      }
