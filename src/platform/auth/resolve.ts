...
export type ResolvedCtx = {
  ...
  orgName: string;
  baseCurrency: string;
  /** IANA org timezone (org.timezone) — freshness times render in it. */
...
/** Per-request memoized (React cache()): layout + page + nested server
 * components share ONE JWT verification per request instead of repeating it. */
...
/** Per-request memoized (React cache()): the org layout, the page and every
 * nested server component resolve the SAME membership once per request — and
 * the returned object identity is stable, so downstream cache()-wrapped reads
 * keyed on `resolved.ctx` (terminology, branding) dedupe too. */
export const resolveCtx = cache(async (orgId: string): Promise<ResolvedCtx | ResolveFailure> => {
  ...
  const membership = await withUserCtx(user.id, async (tx) => {
    const rows = (await tx.execute(sql`
      select m.role_key, o.name as org_name, o.base_currency, o.timezone
      from public.membership m
      join public.org o on o.id = m.org_id
